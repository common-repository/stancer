...
namespace Stancer\Scoped\Isolated\Stancer\Core;
...
abstract class AbstractObject implements JsonSerializable
{
    ...
    {
        $defaultModel = ['allowedValues' => null, 'coerce' => null, 'exception' => null, 'exportable' => null, 'format' => null, 'list' => \false, 'restricted' => \false, 'required' => \false, 'size' => ['fixed' => null, 'min' => null, 'max' => null], 'value' => null];
        $defaultValues = ['created' => ['desc' => 'Creation date', 'restricted' => \true, 'type' => DateTimeImmutable::class]];
        ...
        foreach ($this->dataModel as &$data) {
            ...
            if (\is_a($data['type'], DateTimeInterface::class, \true)) {
     ...
     */
    #[\ReturnTypeWillChange, Stancer\WillChange\PHP8_0\MixedType]
    protected function dataModelGetter(string $property, bool $autoPopulate = \true)
    {
        $model = $this->getModel($property);
        if (!$model) {
            throw new Stancer\Exceptions\InvalidArgumentException(\sprintf('Unknown property "%s"', $property));
        ...
        if (!\is_null($value) && \is_a($model['type'], DateTimeInterface::class, \true)) {
    ...
    {
        $tmp = [Stancer\Config::getGlobal()->getUri(), $this->getEndpoint()];
        if ($this->getId()) {
            $tmp[] = $this->getId();
        }
    ...
    public function hydrate(array $data) : self
    {
        foreach ($data as $key => $value) {
            ...
            if ($property === 'id') {
                ...
                $types = [static::BOOLEAN, static::INTEGER, static::STRING];
                $coerce = function ($v) {
                    return $v;
                };
                $model = $this->getModel($property);
                ...
                if ($value && !\in_array($model['type'], $types, \true) && !\is_object($value)) {
                    $class = $model['type'];
                    if ($model['list']) {
                        $list = [];
                        if (\is_array($value)) {
                            foreach ($value as $val) {
                                if (\is_subclass_of($class, self::class)) {
                                    ...
                                    $missing = \true;
                                    if (!\array_key_exists('value', $this->dataModel[$property]) || !\is_array($this->dataModel[$property]['value'])) {
                        ...
                        if (\is_subclass_of($class, self::class)) {
                            ...
                            if (!\array_key_exists('value', $this->dataModel[$property]) || !$this->dataModel[$property]['value']) {
                                $this->dataModel[$property]['value'] = new $class($id);
                            }
                            if (\is_array($value) && \array_key_exists('value', $this->dataModel[$property]) && $this->dataModel[$property]['value'] instanceof self) {
                                $this->dataModel[$property]['value']->cleanModified = $this->cleanModified;
                                $this->dataModel[$property]['value']->hydrate($value);
                            }
                        } else {
                            if (\array_key_exists('restricted', $this->dataModel[$property]) && $this->dataModel[$property]['restricted']) {
                    ...
                    if ($restricted || \is_null($value) || \is_array($value)) {
                        $this->dataModel[$property]['value'] = $coerce($value);
                    } else {
                        $this->{$property} = $value;
                    }
                ...
                if ($this->populated) {
                    if (\array_key_exists('value', $this->dataModel[$property]) && $this->dataModel[$property]['value'] instanceof self) {
    ...
    public function jsonSerialize()
    {
        ...
        foreach ($struct as $prop => &$value) {
            $supp = !\in_array($prop, $this->modified, \true);
            $model = $this->getModel($prop);
            $format = $model['format'] ?? function ($v) {
                return $v;
            };
            if (\is_object($value)) {
                if (\in_array($prop, $this->modified, \true) || $value instanceof self && $value->isModified()) {
                    $supp = \false;
    ...
    public function populate() : self
    {
        ...
        $body = \json_decode($response, \true);
        $this->cleanModified = \true;
        $this->populated = \true;
        $this->apiData = $body;
        $this->hydrate($body);
        $struct = $this->toArray();
    ...
    {
        $json = [];
        $data = ['id' => ['exportable' => \true, 'value' => $this->id]];
        $data = \array_merge($data, $this->dataModel);
        foreach ($data as $property => $infos) {
            if (\array_key_exists('exportable', $infos) && \array_key_exists('value', $infos) && $infos['exportable'] && $infos['value'] !== null) {
    ...
    protected function validateDataModel(string $property, $value) : self
    {
        ...
        $exceptionList = ['Stancer\\Exceptions\\Invalid' . \ucfirst($property) . 'Exception', $model['exception']];
        ...
        if ($model['allowedValues']) {
            ...
            $allowValue = function ($v) use($model, $names, $property) : string {
                if (!\in_array($v, $model['allowedValues'], \true)) {
                    $params = [$v, $property, $names];
        ...
        if ($isLower || $isUpper) {
            ...
            $params = [$readable, \ucfirst($readable), $model['size']['min'], $model['size']['max']];
            if ($type === 'integer') {
