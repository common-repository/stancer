...
use Stancer\Scoped\Isolated\GuzzleHttp;
use Exception;
...
class Request
{
     ...
     * @return $this
     */
    private function addCallWithDefaultClient(AbstractObject $object, Stancer\Exceptions\Exception $exception = null) : self
    {
     ...
     * @return $this
     */
    private function addCallWithOtherClient(Psr\Http\Message\RequestInterface $request, $response, AbstractObject $object, Stancer\Exceptions\Exception $exception = null) : self
    {
        $config = Stancer\Config::getGlobal();
        $client = $config->getHttpClient();
        ...
        $params = ['exception' => $exception, 'request' => $request, 'response' => $response];
        if ($object instanceof Stancer\Payment) {
            $in = null;
            $out = '';
            $card = $object->getModel('card')['value'];
            ...
            if ($in) {
                $params['request'] = new GuzzleHttp\Psr7\Request($request->getMethod(), $request->getUri(), $request->getHeaders(), \str_replace($in, $out, (string) $request->getBody()));
            }
        }
        $call = new Stancer\Core\Request\Call($params);
        $config->addCall($call);
    ...
    public function request(Stancer\Http\Verb\AbstractVerb $verb, AbstractObject $object, array $options = []) : string
    {
        ...
        try {
            ...
            $params = [$response->getStatusCode(), $response->getReasonPhrase()];
            ...
            if (\is_array($body) && \array_key_exists('error', $body) && \is_array($body['error']) && \array_key_exists('message', $body['error'])) {
