...
namespace Stancer\Scoped\Isolated\Stancer\Http;
...
class Client implements Stancer\Interfaces\HttpClientInterface
{
    ...
    public function request(string $method, string $uri, array $options = []) : Psr\Http\Message\ResponseInterface
    {
        ...
        // Set URL.
        \curl_setopt($this->curl, \CURLOPT_URL, \trim($uri));
        // Set HTTP method.
        \curl_setopt($this->curl, \CURLOPT_CUSTOMREQUEST, \trim($method));
        // Timeout.
        ...
        \curl_setopt($this->curl, \CURLOPT_RETURNTRANSFER, \true);
        ...
        if ($error || $code >= 400) {
            ...
            $params = ['code' => $code, 'request' => $this->lastRequest, 'response' => $this->lastResponse];
            ...
            if ($body->getSize()) {
                ...
                if (\json_last_error() === \JSON_ERROR_NONE && \is_array($json) && \array_key_exists('error', $json) && \array_key_exists('message', $json['error']) && $json['error']['message']) {
